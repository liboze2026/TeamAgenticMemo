...
> 版本: 2.0 | 日期: 2026-04-13 | 状态: 设计完成，待实施
...
```
TeamAgent = MCP Server（核心）
          + Hook脚本（执行臂）
          + Skill命令（用户接口）
          + 知识引擎（大脑）
          + Knowledge Portal（可视化窗口）
```
...
AI建议用REST API，用户说"我们这个场景用GraphQL更合适"。系统识别到纠正时刻，后台提取知识。用户可能看到一条淡淡的提示："已记录经验"，也可能什么都看不到。
...
💡 TeamAgent: 已应用2条相关经验
   • Stripe不支持复合操作，需拆分为独立API调用（置信度0.92，拦截34次）
   • webhook签名验证必须用raw body（置信度0.88，拦截12次）

AI：好的，我来实现。注意这里需要分两步...
```
...
│  ④ 全局视野（智能体自主运行时）                             │
│    "AI会自己发现自己跑偏了"                                │
│    实现: Session Monitor旁路监控 + Prompt动态注入          │
│    覆盖: E1-E2, S1-S4                                    │
│                                                           │
│  ⑤ 持续学习（每次会话结束后）                               │
│    "AI每天都比昨天聪明一点"                                │
│    实现: PostSession分析 — 提取知识/更新置信度/发现模式    │
│    覆盖: 所有类别（离线分析）                               │
│                                                           │
