...
M0  types / ports / Fake adapters / AttributionBus / Renderer  (基石)
 ...
 │
 ├─→ M4  AnthropicLLMClient ─── Extractor (core)
...
6. **CI 集成** — GitHub Actions 每日跑**不需要 LLM 的部分**：M0-M3 的测试 + Matcher/Compiler/Pipeline（LLM 用 mock）。M4+ 的真实 LLM 测试用 `test:llm` 脚本本地/定期人跑，避免 CI 需要 ANTHROPIC_API_KEY 和成本
...
- [ ] Node.js >= 20 已安装
- [ ] pnpm 已安装
- [ ] `ANTHROPIC_API_KEY` 环境变量（M0-M3 不需要；M4 开始前设置即可）
