...
## TeamAgent 经验（87条活跃知识，为你编译了 29 条（token 预算 3000）)
...
- 使用 process.chdir then path.resolve, or C:/bzli/... format 而非 DatabaseSync('/c/bzli/...')——node:sqlite uses Win32 API, does not recognize Git Bash /c/ mountpoint [1.00]
...
- 收到 task-notification 后立即处理输出、推进后续流程——任务完成时收到事件触发，应立即处理而非被动等待全部完成，流式推进提升效率并充分利用并行 [0.90]
