...
// ../cli/src/session-start-logic.ts
...
function isProjectDir(cwd) {
  for (const m of PROJECT_MARKERS) {
    if ((0, import_node_fs.existsSync)((0, import_node_path.join)(cwd, m))) return true;
  }
  return false;
}
function autoInitDisabled(cwd) {
  return (0, import_node_fs.existsSync)((0, import_node_path.join)(cwd, ".teamagent", "auto-init.disabled")) || (0, import_node_fs.existsSync)((0, import_node_path.join)(import_node_os.default.homedir(), ".teamagent", "auto-init.disabled"));
}
function decideAction(cwd, _now, _debounceHours) {
  const dbPath = (0, import_node_path.join)(cwd, ".teamagent", "knowledge.db");
...
function findMainBin() {
  return (0, import_node_path.join)(__dirname, "bin.js");
}
function spawnAutoInit(cwd) {
  const logPath = (0, import_node_path.join)(import_node_os.default.homedir(), ".teamagent", "auto-init.log");
  try {
    (0, import_node_fs.appendFileSync)(
      logPath,
      `[${(/* @__PURE__ */ new Date()).toISOString()}] auto-init spawn cwd=${cwd}
`,
      "utf-8"
    );
  } catch {
...
function logError(kind, err) {
  try {
    const logPath = (0, import_node_path.join)(import_node_os.default.homedir(), ".teamagent", "session-start-errors.log");
    (0, import_node_fs.appendFileSync)(logPath, `[${(/* @__PURE__ */ new Date()).toISOString()}] session-start:${kind} ${String(err)}
`, "utf-8");
  } catch {
  }
}
function teamagentHome() {
  return process.env["TEAMAGENT_HOME"] ?? (0, import_node_path.join)(import_node_os.default.homedir(), ".teamagent");
}
function updateStatePath() {
  return (0, import_node_path.join)(teamagentHome(), "update-state.json");
}
function updateDisabledPath() {
  return (0, import_node_path.join)(teamagentHome(), "auto-update.disabled");
}
function readUpdateState() {
  try {
    if (!(0, import_node_fs.existsSync)(updateStatePath())) return defaultUpdateState();
    return parseUpdateState(import_node_fs.default.readFileSync(updateStatePath(), "utf-8"));
  } catch {
...
function writeUpdateState(s) {
  try {
    import_node_fs.default.mkdirSync(teamagentHome(), { recursive: true });
    import_node_fs.default.writeFileSync(updateStatePath(), serializeUpdateState(s), "utf-8");
  } catch {
  }
}
function shouldSpawnUpdater(now = /* @__PURE__ */ new Date()) {
  return shouldCheckUpdate({
    now: now.getTime(),
    state: readUpdateState(),
    env: process.env,
    disabledMarkerExists: (0, import_node_fs.existsSync)(updateDisabledPath())
  });
}
function spawnUpdater() {
  const updaterBin = (0, import_node_path.join)(__dirname, "bin-updater.cjs");
  if (!(0, import_node_fs.existsSync)(updaterBin)) {
    logError("updater-bin-missing", new Error(updaterBin));
    return;
  }
  const child = (0, import_node_child_process.spawn)(process.execPath, [updaterBin], {
    detached: true,
    stdio: "ignore",
    env: process.env,
    windowsHide: true
...
function maybeShowPendingBanner(stderr = (s) => process.stderr.write(s)) {
...
// ../cli/src/wiki-residue-cleanup.ts
var import_node_fs2 = __toESM(require("fs"), 1);
var import_node_os2 = __toESM(require("os"), 1);
var import_node_path2 = __toESM(require("path"), 1);
function cleanupWikiResidue(homeDir = import_node_os2.default.homedir()) {
  try {
    const target = import_node_path2.default.join(homeDir, ".teamagent", "wiki-refresh-errors.log");
...
async function main() {
  ...
  const action = decideAction(cwd, /* @__PURE__ */ new Date());
  if (action === "auto-init") {
...
`
    ...
    try {
      ...
      logError("auto-init-spawn-failed", e);
    }
