...
// ../cli/src/updater-logic.ts
async function runUpdater(deps) {
  if (!deps.acquireLock()) {
  ...
  try {
    ...
    try {
      ...
      deps.log(`fetch error: ${e.message}`);
      return;
    }
    if (!remoteSha) {
      deps.log("fetch failed or empty");
      return;
    }
    ...
    const banner = { from: fromSha, to: remoteSha, at: deps.now(), shown: false };
    const success = {
      ...state,
...
var defaultHttpsGet = (url, headers) => new Promise((resolve, reject) => {
  const req = import_node_https.default.get(url, { headers, timeout: 1e4 }, (res) => {
    const chunks = [];
...
// ../cli/src/bin-updater.ts
function teamagentHome() {
  return process.env["TEAMAGENT_HOME"] ?? import_node_path.default.join(import_node_os.default.homedir(), ".teamagent");
}
function statePath() {
  return import_node_path.default.join(teamagentHome(), "update-state.json");
}
function lockPath() {
  return import_node_path.default.join(teamagentHome(), "update.lock");
}
function logPath() {
  return import_node_path.default.join(teamagentHome(), "update.log");
}
function rollbackDir() {
  return import_node_path.default.join(teamagentHome(), "rollback");
}
var REPO_OWNER = "libz-renlab-ai";
var REPO_NAME = "TeamBrain";
var REPO_BRANCH = "release";
var PACKAGE_SPEC = `github:${REPO_OWNER}/${REPO_NAME}#${REPO_BRANCH}`;
var BACKUP_KEEP = 3;
...
function readState() {
  try {
    if (!import_node_fs.default.existsSync(statePath())) return defaultUpdateState();
    return parseUpdateState(import_node_fs.default.readFileSync(statePath(), "utf-8"));
  } catch {
...
function writeState(s) {
  ensureDir(teamagentHome());
  import_node_fs.default.writeFileSync(statePath(), serializeUpdateState(s), "utf-8");
}
function acquireLock() {
  ensureDir(teamagentHome());
  try {
    const fd = import_node_fs.default.openSync(lockPath(), "wx");
    import_node_fs.default.writeSync(fd, String(process.pid));
    import_node_fs.default.closeSync(fd);
    return true;
  } catch {
    try {
      const pid = parseInt(import_node_fs.default.readFileSync(lockPath(), "utf-8"), 10);
      if (pid > 0) {
        try {
          process.kill(pid, 0);
          return false;
          ...
          import_node_fs.default.writeFileSync(lockPath(), String(process.pid), "utf-8");
          return true;
        }
      }
    } catch {
...
function findGlobalDistDir() {
  const candidate = __dirname;
  if (import_node_fs.default.existsSync(import_node_path.default.join(candidate, "bin.js"))) return candidate;
  return null;
}
function copyDirSync(src, dest) {
  import_node_fs.default.mkdirSync(dest, { recursive: true });
  for (const e of import_node_fs.default.readdirSync(src, { withFileTypes: true })) {
    const s = import_node_path.default.join(src, e.name);
    const d = import_node_path.default.join(dest, e.name);
    if (e.isDirectory()) copyDirSync(s, d);
    else import_node_fs.default.copyFileSync(s, d);
  }
}
function backupCurrentInstall(oldSha) {
  ensureDir(rollbackDir());
  const dist = findGlobalDistDir();
  if (!dist) return "";
  const tag = oldSha || `pre-${Date.now()}`;
  const dest = import_node_path.default.join(rollbackDir(), tag);
...
function restoreFromBackup(backupDir) {
  if (!backupDir || !import_node_fs.default.existsSync(backupDir)) return;
  const dist = findGlobalDistDir();
  if (!dist) return;
  try {
    import_node_fs.default.rmSync(dist, { recursive: true, force: true });
...
function pruneOldBackups() {
  if (!import_node_fs.default.existsSync(rollbackDir())) return;
  try {
    const entries = import_node_fs.default.readdirSync(rollbackDir()).map((name) => ({ name, mtime: import_node_fs.default.statSync(import_node_path.default.join(rollbackDir(), name)).mtimeMs })).sort((a, b) => b.mtime - a.mtime);
    for (const entry of entries.slice(BACKUP_KEEP)) {
      import_node_fs.default.rmSync(import_node_path.default.join(rollbackDir(), entry.name), { recursive: true, force: true });
    }
  } catch {
  }
}
function runNpmInstall() {
  return new Promise((resolve) => {
    ...
    const child = (0, import_node_child_process.spawn)(npm, ["install", "-g", PACKAGE_SPEC], {
...
function runMigrateAuto() {
  return new Promise((resolve) => {
    const dist = findGlobalDistDir();
    if (!dist) return resolve({ ok: true });
    const binJs = import_node_path.default.join(dist, "bin.js");
    if (!import_node_fs.default.existsSync(binJs)) return resolve({ ok: true });
    const child = (0, import_node_child_process.spawn)(process.execPath, [binJs, "migrate-auto"], {
      stdio: ["ignore", "pipe", "pipe"]
    });
    let err = "";
    child.stderr?.on("data", (d) => {
...
main().catch((e) => log(`updater crash: ${e.message}`));
