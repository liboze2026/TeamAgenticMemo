...
describe("runSkeletonDemo", () => {
  ...
  it("smart mode (default) → includes attribution block header", () => {
    const out = runSkeletonDemo({ env: {}, now: fixedNow });
    expect(out).toContain("✨ TeamAgent");
    expect(out).toContain("本次操作归因");
    expect(out).toContain("[skeleton]");
  });

  it("smart mode does NOT include counterfactual line", () => {
    const out = runSkeletonDemo({ env: {}, now: fixedNow });
    expect(out).not.toContain("如果没有 TeamAgent");
  });

  it("silent mode → empty output", () => {
    const out = runSkeletonDemo({
  ...
  it("verbose mode → includes counterfactual + raw JSON", () => {
    const out = runSkeletonDemo({
  ...
  it("unknown mode falls back to smart", () => {
    const out = runSkeletonDemo({
